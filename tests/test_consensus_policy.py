...
现在：**收敛判据**是结构化的（无 `reject` + 至少一位 `approve` + 支持度 ≥ threshold），
**终态分类**由决策构成直接给出。
"""
...
def _judge(decisions: list[str], *, final: bool) -> str:
    ...
    opinions = {e: _op(e, d) for e, d in zip(experts, decisions)}
...
# --------------------------------------------------------------------------- #
# 第 2 层：停止时如何分类（final=True）
# --------------------------------------------------------------------------- #

MATRIX: tuple[tuple[str, list[str], str], ...] = (
    # 全 revise：一致认为方向可行、需先满足条件 —— **不是**「分歧未解决」
    ("全 revise（n=3）", ["revise"] * 3, "conditional"),
    ("全 revise（n=6）", ["revise"] * 6, "conditional"),
    ...
    # 无反对 + 有明确赞成 → 通过（与激活了几位专家无关）
    ("1 approve + 其余 revise（n=3）", ["approve", "revise", "revise"], "approved"),
    ("1 approve + 其余 revise（n=5）", ["approve"] + ["revise"] * 4, "approved"),
    # 支持度 1/6 = 0.167 < 0.6 → 「附条件交付」而不是「通过」：五位专家都只肯说「先满足条件」，
    # 把它标成 approved 会高估支持度。**残余的 n 敏感性就留在这一档**（approved↔conditional），
    # 不再影响「交给人还是交付」；是否该把门槛改成票数口径见 Q-26。
    ("1 approve + 其余 revise（n=6）", ["approve"] + ["revise"] * 5, "conditional"),
    ("全 approve（n=4）", ["approve"] * 4, "approved"),
    # 支持度不足（1 赞成 + 4 弃权 → 0.2）→ 附条件交付，而不是「通过」
    ("1 approve + 4 判断性弃权", ["approve"] + ["abstain"] * 4, "conditional"),
)
...
def test_all_revise_is_the_same_verdict_for_any_panel_size() -> None:
    """优化前：n=3 与 n=6 都是 0.5000（二义性）；现在两者都明确是「有条件交付」。"""
    assert _judge(["revise"] * 3, final=True) == "conditional"
    assert _judge(["revise"] * 6, final=True) == "conditional"
...
def test_one_approver_no_longer_flips_with_panel_size() -> None:
    ...
    优化后：两种规模都**不再交人裁定**（阻断类判据只看「有没有人反对」，与规模无关）；
    差别只剩「明确支持够不够」这一档：n=5 支持度 0.20 达标 → `approved`，n=6 为 0.167 →
    `conditional`（附条件交付）。残余的规模敏感性被限制在**交付强度**上，见 Q-26。
    """
    assert _judge(["approve"] + ["revise"] * 4, final=True) == "approved"
    assert _judge(["approve"] + ["revise"] * 5, final=True) == "conditional"


def test_a_deadlock_is_not_reported_as_agreement() -> None:
    """优化前「半赞成半反对」与「全 revise」同为 0.5000，无法区分；现在前者是 manual_review。"""
    assert _judge(["approve", "approve", "reject", "reject"], final=True) == "manual_review"
    assert _judge(["revise"] * 4, final=True) == "conditional"
...
def test_quorum_and_evidence_gap_are_terminal_at_once() -> None:
    ...
    assert consensus(all_gap, weights, THRESHOLD, MIN_EFFECTIVE, final=False)[2] == "insufficient_evidence"
...
class _ConditionalLLM:
    """全员 revise + 各自列出前置条件：复现「方向可行、条件待满足」的真实形态。"""
    ...
    async def complete(self, *, purpose: str, system: str, user: str, meta=None):  # type: ignore[no-untyped-def]
        ...
        return LLMResult(content=content, model="conditional", usage=Usage(calls=1))


def test_conditions_travel_with_the_deliverable() -> None:
    """终态是 `conditional`，且前置条件进入**结构化契约**与报告，而不只是散文。"""
    ctx = RunContext(
        run_id="conditional",
        llm=_ConditionalLLM(),  # type: ignore[arg-type]
    ...
    # 那会得到 `stalled`；本用例要验证的是「一轮结束、无人反对 → 附条件交付」。
    result = asyncio.run(run(RunInput(request="301分段FR36污水井更换加厚板"), ctx, max_rounds=1))
