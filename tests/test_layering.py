...
这两条是 AGENTS.md §1.2 列的架构不变量，此前**没有任何测试**守着——只有文档里的
一句话。十模块重构把它们变成可机检的性质：目录名就是层名，跨层 import 就是违规。

顺带解决一个真实风险：重构本身错了（比如 `rag` 反向 import `workflow`），
`ruff` 与功能测试都不会红——功能照样跑，只是依赖方向悄悄烂掉。
"""
