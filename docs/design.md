...
*最近一次修订：缺口回填 —— 证据请求真的会在下一轮扩展基线（D-97）。修订要点：缺口通道此前只建了**上报**半条（D-94 能把请求登记下来、`effective_round = N+1` 也早已写好），却**没有任何东西消费它**：`prefetch` 每 run 只跑一次、query 来自请求词而非缺口词，于是第 2、3 轮冻结同一批 id、「无新信息」恒真、缺口永远只能记 `evidence_request_unsatisfied`。现在外环在**每轮派发之前**用缺口自己的确定性 query 逐条重跑检索并登记，本轮冻结基线因此包含新证据且**全体专家可见**；请求新增 `satisfied_round` / `satisfied_evidence`，终局分「已回填 / 已检索未命中 / 未回填」三态汇报；`RevisionContext.new_evidence_ids` 由两次冻结基线相减得出并**渲染进 prompt**（此前恒为空且根本没被渲染）；终局 `grounding` 按回填后的证据重算。同时把「是否给子智能体自主检索能力」（D-11 的 `search()` 侧）登记为 **Q-28** 待定，并删掉 `hasattr(retriever, "search")` 那处掩盖差异的探测。再上一轮为共识状态收成三个取值、「交人原因」移入 `review_reason`（D-96）。修订要点：D-93 / D-95 各自新增的状态取值（`insufficient_evidence` / `conditional`）**取消**——它们区分的四种情形本来就在专家意见里，而每个取值都要求所有消费者多写一条**没有行为差**的分支（收束 act 一直只认 `approved` / `stalled`，`conditional` 从未因此自动交付）。现在状态值只回答「交付还是交人」，原因由 `RunResult.review_reason` 承载，报告新增「交人原因」行；区分一条不丢，并新增了「缺席（`quorum`）」这条此前在契约里不可辨认的信息。`stalled` 按 D-82 保留。再上一轮为共识判定分两层（D-95）：标量分数从**判据**降为**描述性支持度**，阻断类判据（缺席 / 证据缺口 / 有人反对 / 无人赞成）全部改为结构化构成判断。再上一轮为弃权分类与证据缺口通道（D-93 / D-94）。再上一轮为专家评审技能化 + 弃权策略修正（D-92）。再上一轮为 harness 竖切（D-89…D-91），更早为十模块布局（D-88）、振荡检测（D-87）、调用上下文外移（D-86）与元智能体架构（D-70…D-85）。*
