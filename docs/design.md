...
> **实现现状（D-89…D-91 / D-102）**：循环、六个 act、守卫判据与 `StepRecord` 已落地；决策部分是**三明治**——规则骨架先出激活集与权重，LLM 只在**首轮之前**补一次差集（含证据子集分配），合并与校验由代码负责，不可用即退回纯骨架（D-102）。裁定（共识 / 停滞）由外环以回调注入，内环因此不 import 外环。
...
`[已定]` 规则 + LLM 混合：`disciplines` 命中达阈值的专业由**规则直接激活**（可复现），LLM 只处理规则未覆盖的部分。每条激活须带 `rationale` 与引用的 `evidence_ids`。

落地形态见 **D-102**：骨架先产出完整的 `ActivationPlan`，LLM 在首轮之前补一次差集（激活集 / 权重 / 证据子集），
「只增不减 + 归一化 + 与基线求交」由 `meta.merge_decision` 这段代码完成，越界与失败一律退回纯骨架并落事件。
`rationale` 只进事件日志与审计，**不进任何子智能体 prompt**（D-76）。
...
| `manual_review` + `review_reason="conditions_only"` | ✅ 生成，标**未定稿**（**D-101**） | 无人明确反对 ⇒ 有方向可写；但「无人反对」≠「可以施工」，前置条件必须逐条落条 |
| `manual_review`（其余原因：分歧 / 缺席 / 证据缺口） | ❌ **不生成** | 没有方向可写，是**回到中段**（见 §5.9） |
...
| 层 | 生命周期 | 内容 | 谁看得到 |
| --- | --- | --- | --- |
| 会话上下文 | 跨对话轮 | 用户原话、已确认范围、约束、历史结论摘要 | 元智能体（决策层 L1）与方案撰写者（D-98 / D-103）；**评审专家看不到** |
...
| 会话历史 | **合法读者**（决策层 L1） | **评审专家完全不可见**；方案撰写者按 D-98 可见（D-103） |
...
| D-101 | **修正 D-98 的出方案条件**：`manual_review` + `review_reason="conditions_only"` 也产出方案，并在交付物头部标**未定稿**（前置条件未落实、需人工确认）；其余交人原因（分歧 / 缺席 / 证据缺口 / `unsupported_plan`）与 `stalled` 仍不出。同时把 `LLM_MAX_TOKENS` 默认值 2048 → 8192，并在方案提示词里加**规模上界**（claims 合计 ≤ 24 条、单条 ≤ 120 字、每节正文 ≤ 200 字、同一件事合并） | **真实模型实测推翻了 D-98 的原判**：累计 5 次真实 run（含「污水井位置变更」案例）专家**从不 approve**——三轮一致 `revise`、支持度 0.50、`conditions_only`。若只有 `approved` 出方案，框架在真实运行里**永远不产出交付物**，闭环纸面上成立、实践上不成立。而 `conditions_only` 的语义（§5.6.1）恰是「无人明确反对、方向可行、需先满足前置条件」——**这正是方案撰写者该写的东西**，只是必须标未定稿（「无人反对」≠「可以施工」）。另一半是**实测出来的工程约束**：方案是所有产物里最长的，`max_tokens=2048` 把方案 JSON 截断在半句上（`Expecting ',' delimiter ... char 3507`），提到 4096 仍不够（截断点移到 `char 7943`），契约重试耗尽后**回落模板**——降级路径本身是对的，但交付物质量掉下来了。8192 **加上**提示词的规模上界之后，同一请求的真实 run 拿到 `plan_source="agent"`：6 节、27 条、全部条目可回溯、20 条前置条件全在方案里 | `[已定]` |
| D-102 | **落地决策层的第二层**（Q-06 三明治的「LLM 补差集」）：只在**首轮之前**跑一次 LLM，输入 = 骨架的 `ActivationPlan` + 请求 + 图 `disciplines` + 有界证据视图（≤ `MAX_DECISION_EVIDENCE`）+ **会话快照**（L1）；输出 = 闭集 JSON `{add_experts, weights, evidence_scope, rationale}`（契约 `MetaDecision`）。合并/校验/归一化由 `meta.merge_decision` 这段代码完成：**只增不减**、权重归一化到 Σ=1、证据子集与基线求交（空集回落整份基线）。越界（未知专家 id / 本轮不可引用的证据 id / 负权重）在解析层抛 `ContractViolation` → 带反馈重试一次 → 耗尽即**整层退回纯骨架**，落 `degradation(level="rule_skeleton")` 事件与 `meta_decision_failed` 警告；裁剪落 `evidence_scope_trimmed` 事件（§5.4.3 的 L3）；开关 `META_DECISION=llm\|rule`，默认 `llm` | **为什么只第 1 轮**：激活决策做一次就够；每轮重做会让激活集自己振荡，且每轮多一次付费调用。**为什么骨架的差集只能加不能删**：骨架是可复现的底线，让模型有权「删」等于把一个概率组件放进可达性路径上；只增不减也把「漏专家 → 漏证据 → 漏请求」这条最贵的错挡在骨架那一侧。**为什么越界必须退回而不是静默裁剪**：静默丢弃会让「模型编了个 id」在产物里看不出来，而证据可回溯（P7 / §5.7）依赖这条链干净。**为什么证据子集只作用首轮**：第 2 轮起基线会变，沿用一份按首轮裁过的子集，等于让专家看不到新证据——那时回到整份当前基线。**为什么离线适配器返回空差集**：offline 的产物必须与接入之前逐字一致，否则「offline 是判定基准」这条不成立 | `[已定]` |
| D-103 | **会话快照的读者按角色划**（修正 §8.4.5/§6.3 与 D-98 的矛盾）：合法读者是**元智能体（决策层，L1）**与**方案撰写者**（D-98 的输入契约明写「会话上下文」）；**评审专家仍然完全不可见**。§8.4.5 的旧措辞「只有元智能体」写在 D-98 之前，未随方案生成节点更新 | 与 D-100 同一条思路：匿名披露、会话可见性都是**按角色**划的规则，不是全局开关。评审专家看不到会话历史，理由是独立判断不能被上一轮结论污染（§8.4.5 的本意）；而汇总者与决策者需要跨轮连续性——否则「同一会话的下一轮」与「第一次对话」没有区别，D-43 的会话快照也就白留了。把规则写成「谁不可见」而不是「只有谁能见」，是为了下一次新增角色时不必再改这条不变量 | `[已定]` |
...
| Q-06 | meta_agent 决策方式 | 纯 LLM / 规则+LLM 混合 / 循环 | **已定且已落地（D-70/D-73，D-102）**：元智能体是 run 级常驻的 think-act-observe 循环；决策部分用**三明治**——规则先出骨架（`disciplines` 达阈 / 关键词先验 / 最小专家数），LLM 只补骨架未覆盖的差集，代码负责合并、校验、归一化；LLM 不可用或越界即退回纯骨架 + `degradation` 事件 |
...
| Q-31 | **agentic RAG（rag 模块内的 text2Cypher）** | 维持三条固定查询 / 模块内加 agentic 生成（skill + 守卫 + 只读事务 + 配额）/ 二者混合（固定查询保基线，agentic 只做增量） | **本轮只加固、不实现**（已就位的加固见 §1.2 的只读入口与降级路径）。按「模块内部替换」理解：`RetrieverPort` 签名不变，但接口今天**默默承载**的三条义务必须由实现主动挣得——① **可重放**：生成查询与结果落事件日志，重放按记录走、不重新生成（P7 / D-86 / §5.4.4 的不动点证明依赖「同输入同基线」）；② **失败回落**：生成的查询可能「不报错但查空」，必须回落固定查询并**显式降级**（P5），否则空基线会伪装成「没有历史案例」；③ **查询溯源**：日志记 query→evidence_ids，否则无法解释某条证据为何进基线（§1.2 事实可回溯）。另有两点前置：`rag/schema.py` 做成图谱 schema 的唯一定义处（skill 只引用不复制，配防漂移测试）；`tests/test_layering.py` 的「Cypher 只在两个文件」需换成「Cypher 只有一个执行入口且必经 `CypherGuard`」——生成式查询让旧形态不再成立。**与 Q-28 无关**：Q-28 问「子 agent 能否自主检索」，本项是 rag 模块内部实现方式的替换 |
