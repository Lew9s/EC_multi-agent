...
**落地（D-97）**：这张图的下半段此前没有实现——请求登记得下来，`effective_round = N+1` 也早就写好，但**没有任何东西消费它**，于是「下一轮基线上扩」从未发生。现在由外环的 `refill_evidence()` 接上：它在**每轮派发之前**把到期（`effective_round <= N`）且从未尝试过的请求，用它们**自己的确定性 query**（D-94 的模板，不是模型原文）逐条重跑 `prefetch`，登记进 registry；守卫紧接着冻结的本轮基线因此可能包含新证据，**全体专家同时可见**。

**回填的四种终局**（每种对应不同的补救动作，不能糊成一句）：`未回填`（无检索端 / 轮次已用尽）｜`语料未命中`（`hits = 0`，库里确实没有）｜**`命中的都已在本轮基线中`**（`hits > 0` 而新增 `= 0`：**要补的是语料，不是再检索一次**）｜`已回填新证据`。第三与第四种曾共用一句「已回填 N 条」（用的是命中数），于是「基线零增量」被写成「补到了 3 条」——真实 run 的形态恰恰就是「命中 3 条、新增 0 条」，把命中数当回填数会**正好说反**。因此 `satisfied_hits`（命中）与 `satisfied_evidence`（真正让基线变大的）是两个字段。

**这与 `search()` 无关**：缺口回填是**外环**的确定性事实通道（门禁与基线口径不变，D-15）；`search()` 是专家自主的**解释性**检索通道，服务于 L1/L2，见 Q-28。
...
`consensus_status` 只回答**「交付还是交人」**（D-82 / D-96）；**「因为什么交人」由 `review_reason`
回答**：

| 取值 | 含义 | 补救动作 |
| --- | --- | --- |
| `approved` | 共识达标：无 `reject` + 至少一位 `approve` + 支持度 ≥ threshold | 交付 |
| `manual_review` | **交人裁定** —— 原因见 `review_reason` | 人工裁定 |
| `stalled` | **流程已无信息增益**（不动点：无新证据 + 零改变率） | 停并交人（`stall.stalled` 另行记录） |

| `review_reason` | 什么时候 | 交给人时要说什么 |
| --- | --- | --- |
| `quorum` | 有效专家 < 下限（有专家**没交出**意见） | 「不是分歧，是缺席」——查清原因后重跑或补评审 |
| `disagreement` | 有人 `reject` | 「专家未能达成一致，请裁」 |
| `conditions_only` | 无人反对，但支持度不足以自动交付（多为全员 `revise`） | 「方向可行，条件先满足」——附 `conditions` 清单（D-95） |
| `evidence_gap` | 全员判断性弃权 | 「证据不足以评估」——附结构化缺口清单（D-93 / D-94） |

**为什么状态值只有三个**（D-96 推翻 D-93 / D-95 的取值部分）：「交人」的四种理由**本来就在专家
意见里**（有人反对 / 全部附条件 / 全员弃权 / 有人缺席），各做成一个状态取值等于要求**每个**消费者
为每种理由各写一个分支，而这几个分支**没有对应的行为差**——收束 act 一直是
`closing(status == "approved", status == "stalled")`，`conditional` 从未因此自动交付，它实际也在
问人（状态值与实际行为不一致）。状态值只该说「交给谁」；原因、条件、缺口清单各有字段。

**`stalled` 仍然并行的理由**（D-82 未变）：它在「交给谁」这一层确实不同——`manual_review` 是
「需要人介入裁定」，`stalled` 是「流程已停、再跑也不会有新信息」；且它在 `RunResult` 里另有
`stall.stalled` 作为结构化载体，不是靠状态值单独承载信息。
...
| D-92 | 专家评审抽成**目录形式的技能包**（`agents/skills/expert_review/`：`SKILL.md` + prompt / personas / runner）；**专业判断可基于领域通识**，`abstain` 收窄为「超出本专业范围 / 请求本身无法判定」；新增 `ExpertOpinion.basis` 并据此**降级保证等级** | 真实 run 曾出现「语料是变更单台账、缺技术细节 → 4 位专家全体 `abstain` → 共识分 0 → 第 1 轮即 `manual_review`」，**迭代与证据请求双双失效**。根因是提示词把 P3（事实须可回溯）扩大成「没有案例证据就不能判断」，把**专业判断**也一并禁掉；而设计本来就有 `knowledge_based` 这一档（§5.5.1 / §5.5.6 / D-42）：无历史**也要评估**，只是降级标注。修正后同一请求的真实 run：`abstain ×4 → revise ×4`、轮次 1 → 3、共识分 0 → 0.50，`attribute` 与匿名披露首次真正执行。另：保证等级此前只看「检索有没有命中」，不看专家实际用了什么依据（真出现过 4 位全凭通识却报 `history_backed`），`basis` 修掉这个不一致。技能做成目录包而非散落在 `experts.py`，是为了让「方法」可被多个 agent 复用，而「谁」留在专家目录 | `[已定]` |

| D-93 | 弃权分两类：**判断性弃权**（模型在契约内说「我无法结论」）与**执行失败弃权**（超时/传输/契约重试耗尽）；由**服务端**在构造点判定、模型不可伪造。quorum 的「有效专家」改为**交付了意见的专家**，只有执行失败才算缺席；`consensus_status` 增第四取值 `insufficient_evidence`（全员判断性弃权），且该状态下 `assurance` 强制 `knowledge_based`（**该取值已于 D-96 取消**：终态收为三个，理由移入 `review_reason="evidence_gap"`，`assurance` 的降级判据随之改为按原因标签） | D-37 的原意是「避免**缺席**被当作通过」，落地时却把「缺席」等同于了 `abstain` 这个值——于是「四位专家一致说证据不足」被读成「四位专家缺席」，quorum 失败、第 1 轮即 `manual_review`，迭代与证据请求双双失效（真实 run 实测）。两类弃权的补救动作相反：补证据 vs 重试/换模型，所以必须分开（与 D-82 把 `stalled` 独立出来同一纪律）。四种状态的补救动作互不相同：`approved` 交付 / `manual_review` 人工裁定分歧 / `stalled` 流程已停 / `insufficient_evidence` 补证据。该状态不硬迭代（D-29 / §5.5.5：无新信息时迭代只产生共识幻觉）；`assurance` 降级是因为**本轮没有任何专业判断**，检索命中历史案例不等于有人用它作了结论 | `[已定]` |
| D-94 | 证据缺口由专家写好的待补清单（`uncertainties` / `constraints`）**确定性派生**为 `EvidenceRequest`：固定**词表 → scope** 映射 + 模板 query（原始请求 + 命中词），**绝不把模型原文当检索输入**；每轮上限 2 条 / 每条 4 词；记账幂等；缺口进入 `RunResult.evidence_requests` 与报告专节 | 触发信号此前挂在「归因结论 = 证据不同」上，而全员弃权时**没有分歧可归因**，那条路在实践中不可达——「缺什么」明明就写在专家意见里，却只沉在渲染文本中（既没进状态机、也没进结构化产出）。把模型文本直接当 query 会重开「概率输入 → 检索结果 → 下一轮事实基线」的通道，与 D-71 同一精神相悖；词表拼接则检索行为可复现。检索端（`RetrieverPort.search()`）未落地时，请求显式记为 `evidence_request_unsatisfied`并落降级事件（P5） | `[已定]` |
...
| D-97 | **缺口回填**：外环在**每轮派发之前**（经 `refill_fn` 回调注入内环）把「已到生效轮次、且从未尝试过」的 `EvidenceRequest` 用其**确定性 query**（D-94 模板）**逐条**重跑 `prefetch` 并登记进 registry——守卫紧接着冻结的本轮基线因而可能包含新证据且**全体专家可见**（§5.2.4 的落地）。请求新增 `satisfied_round` / `satisfied_hits` / `satisfied_evidence`；终局按「**未回填** / 已检索但**语料未命中** / 已检索但**命中的都已在本轮基线中** / **已回填新证据**」四态分别汇报（`evidence_request_unsatisfied` / `evidence_request_no_hit` / `evidence_request_known_only`）；`RevisionContext.new_evidence_ids` 由两次冻结基线相减得出**并渲染进 prompt**；终局 `grounding` 按回填后的证据重算。删掉 `hasattr(retriever, "search")` 那处探测 | 缺口通道此前只建了**上报**半条：请求登记得下来、`effective_round = N+1` 也早已写好，却**没有任何东西消费它**——`prefetch` 每 run 只跑一次、query 来自请求词而非缺口词，于是第 2、3 轮冻结同一批 id，「无新信息」恒真、缺口永远只能记 `evidence_request_unsatisfied`。D-17「事实性检索必须走请求通道」因此停在半空：请求能提，没人去查。**逐条**检索而非合并成一批，是因为 `EvidenceBundle` 不带「哪条命中来自哪条 query」，合并调用就回答不了「这条缺口补到了什么」。**hits 与 new 必须分开**：把「命中数」当成「回填数」，会把「基线零增量」写成「已回填 3 条」——正好说反。**但要注意这组数字在什么后端上才有意义**：一次真实 run（`run_id=606472b6c985`）报「命中 3 条、新增 0 条」，事后按内容哈希核对，那 3 条**不是检索结果**，而是 `InMemoryRetriever` 的构造数据（`auto` 两次降级后落到内存 fixture，而它的 `prefetch()` **忽略 query**、恒返回同样 3 条）。因此 `run()` 现在会为 fixture 后端落 `retriever_is_fixture` 警告：一次「本来就是演示数据」的 run 不该看起来像「命中了历史案例」。另一端同样致命——**当前唯一可用的图腿根本服务不了缺口 query**：`_CYPHER_PREFETCH` 把每个 query 当作「图节点名的子串」匹配，实测整段请求与缺口 query 都是 0 命中，只有裸实体名（`301分段` / `FR36`）各命中 3 条。也就是说 D-94 的 query 形态只对**向量腿**成立（见 Q-29），「补不进新证据」既不是语料问题、也不是 query 不够不同的问题。另外两处连带：`new_evidence_ids` 此前恒为空**且根本没被渲染**，基线扩张后专家看到的只是「更大的一堆证据」，无从知道本轮该重新检讨什么；回填**不追溯**改变入口的门禁判定（那个决定基于第 1 轮基线做出、HITL 成本已付），但终局的依据等级与保证等级必须跟着证据走（§5.7）。`search()` 不在本项范围内——它服务于 L1/L2 的**解释性**检索，见 Q-28 | `[已定]` |
...
| Q-27 | 检索后端降级时 `assurance = history_backed` 的依据是什么 | 只认图谱侧历史案例 / 降级即强制 `knowledge_based` / 分别记录两条通道 | **已定位**：D-96 那次 run 的降级**不是**「图腿命中」，而是 `auto` 两次降级后落到了 `InMemoryRetriever`——它的条目自带 `source="graph"`，`assess_grounding()` 因此报 `history`（按证据 id 的内容哈希逐一核对过：4 个 id = 请求文本 + fixture 的 3 条构造数据）。**这确实是高报**：一次演示数据 run 报了 `history_backed`。已加 `retriever_is_fixture` 告警（D-97）。**仍未定**的部分：真实后端（图腿）可用、只有向量腿降级时，保证等级该怎么算 |
| Q-28 | 是否给子智能体检索能力（D-11 的 `search()` 侧） | 落地 L1/L2 + `search()` 工具（配额 / 缓存 / 只回摘要 / 轨迹回放，§5.2.3 四件套）/ 明确宣告不做 agentic 检索并把 D-11 降级为单接口 | 现状是**悬着**：契约写着双接口、端口只实现了 `prefetch`、还有一处 `hasattr(retriever, "search")` 探测在掩盖差异（D-97 已删掉那处探测）。专家一律 L0（单次调用、不依赖 provider 的 tool calling），没有 L1/L2 循环，所以 `search()` 即使实现也**没有调用方**。两条路必须选一条：要做就连配额与轨迹回放一起做；不做就该把 D-11 降成单接口并改文档——悬着是唯一不能选的。**缺口回填（D-97）走的是外环通道，与这条独立**，所以本项可以安心待定 |
| Q-29 | 缺口 query 该按哪种检索端的口径构造（D-94 的 query 形态 vs `Neo4jRetriever` 的匹配规则） | 按后端分别构造（图腿用实体名/词表词，向量腿用长文本）/ 缺口 query 改为**多词列表**而不是拼接成一条 / 缺口回填只允许走向量腿，图腿显式拒绝并告警 | **实测的接口错配**（`mode=graph`，真机 118 个变更单）：`_CYPHER_PREFETCH` 的判据是 `co.name CONTAINS term OR any(c IN components WHERE c CONTAINS term) OR …`，即每个 query 必须**是某个图节点名的子串**。于是 `"301分段"`、`"FR36"` 各命中 3 条，而整段请求、D-94 的缺口 query、`"301分段 FR36 污水井"` **全部 0 条**——缺口回填在当前唯一可用（向量腿的 embedding 端不可用）的后端上**必然 hits=0**，与语料是否丰富无关。第二条实测：三条缺口的 query 彼此**不同**，但命中集合相同（图腿只看实体名）。注意 `cases_by_terms(queries, limit)` 内部是 `any(term IN $terms …)`，所以**把词作为列表项传进去是有救的**，问题出在 D-94 把它们拼成了一条字符串。本项直接决定「补充资料」机制能不能真的取到东西，优先级高于 Q-25 / Q-26 |
...
*最近一次修订：缺口回填 —— 证据请求真的会在下一轮扩展基线（D-97）。修订要点：缺口通道此前只建了**上报**半条（D-94 能把请求登记下来、`effective_round = N+1` 也早已写好），却**没有任何东西消费它**：`prefetch` 每 run 只跑一次、query 来自请求词而非缺口词，于是第 2、3 轮冻结同一批 id、「无新信息」恒真、缺口永远只能记 `evidence_request_unsatisfied`。现在外环在**每轮派发之前**用缺口自己的确定性 query 逐条重跑检索并登记，本轮冻结基线因此可能包含新证据且**全体专家可见**；请求分开记 `satisfied_hits`（命中）与 `satisfied_evidence`（真正新增），终局分**四态**汇报（未回填 / 语料未命中 / 命中的都已在本轮基线中 / 已回填新证据）；`RevisionContext.new_evidence_ids` 由两次冻结基线相减得出并**渲染进 prompt**（此前恒为空且根本没被渲染）；终局 `grounding` 按回填后的证据重算。同时把「是否给子智能体自主检索能力」登记为 **Q-28**、把「缺口 query 与图腿的接口错配」登记为 **Q-29**（实测：图腿把每个 query 当「图节点名的子串」匹配，缺口 query 必然 0 命中，只有裸实体名命中），并删掉 `hasattr(retriever, "search")` 那处掩盖差异的探测。**另修正一处错误归因**：上一版把某次真实 run 的「命中 3 条、新增 0 条」读成「语料不够」，按内容哈希核对后发现那次用的是 `InMemoryRetriever`（`auto` 两次降级）——它忽略 query、恒返回 3 条构造数据，且条目自带 `source="graph"`，这同时回答了 Q-27（保证等级被高报）。为此新增 `retriever_is_fixture` 告警，并把原 query 集与缺口 query 都写进事件日志：**可观测性缺口正是这次误读的成因**。再上一轮为共识状态收成三个取值、「交人原因」移入 `review_reason`（D-96）。修订要点：D-93 / D-95 各自新增的状态取值（`insufficient_evidence` / `conditional`）**取消**——它们区分的四种情形本来就在专家意见里，而每个取值都要求所有消费者多写一条**没有行为差**的分支（收束 act 一直只认 `approved` / `stalled`，`conditional` 从未因此自动交付）。现在状态值只回答「交付还是交人」，原因由 `RunResult.review_reason` 承载，报告新增「交人原因」行；区分一条不丢，并新增了「缺席（`quorum`）」这条此前在契约里不可辨认的信息。`stalled` 按 D-82 保留。再上一轮为共识判定分两层（D-95）：标量分数从**判据**降为**描述性支持度**，阻断类判据（缺席 / 证据缺口 / 有人反对 / 无人赞成）全部改为结构化构成判断。再上一轮为弃权分类与证据缺口通道（D-93 / D-94）。再上一轮为专家评审技能化 + 弃权策略修正（D-92）。再上一轮为 harness 竖切（D-89…D-91），更早为十模块布局（D-88）、振荡检测（D-87）、调用上下文外移（D-86）与元智能体架构（D-70…D-85）。*
