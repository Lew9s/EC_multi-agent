...
| 证据请求通道（**回填**侧） | **机制已通**（D-97）：外环在每轮派发之前用缺口自己的确定性 query 重跑 `prefetch`，登记进 registry，本轮冻结基线因此可能包含新证据且全体专家可见。请求分开记 `satisfied_hits`（命中）与 `satisfied_evidence`（真正新增），终局分**四态**汇报：未回填 / 语料未命中 / 命中的都已在本轮基线中 / 已回填新证据。 |
| **缺口 query 与图腿不匹配（实测，待修）** | 实测（`mode=graph`，真机 118 个变更单）：`_CYPHER_PREFETCH` 把每个 query 当作**「图节点名的子串」**匹配，因此 `"301分段"` / `"FR36"` 各命中 3 条，而**整段请求、缺口 query、`"301分段 FR36 污水井"` 全部 0 条**。缺口 query（D-94：整段请求 + 追加词）的形态只对**向量腿**成立；图腿结构上无法服务它，**与语料是否丰富无关**。当前 embedding 端不可用 → `auto` 只剩图腿 → 缺口回填必然报 `hits=0`。见 design.md **Q-29**。 |
