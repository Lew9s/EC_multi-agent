...
> **为什么单列这条**：真实 run 曾出现「语料是变更单流水台账、没有板厚/材质/WPS 参数 →
> 4 位专家全体 `abstain`」。那不是模型的错，是提示词把 P3（事实须可回溯）扩大成了
> 「没有案例证据就不能判断」，把**专业判断**也一并禁掉了。而设计本来就有 `knowledge_based`
> 这一档（§5.5.1 / §5.5.6 / D-42）：无历史**也要评估**，只是降级标注。
...
3. **弃权**：修复也做不到 → `abstain_opinion()`，`uncertainties` 里带原始错误。
   ⚠ 注意这与「模型自报弃权」目前共用 `decision="abstain"`：**判断性弃权 vs 执行失败弃权
   的区分**是下一步（见 `docs/design.md` D-92 计划位）。
